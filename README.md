...
- **CPA 对话**用到 `100 - reserve_percent`（默认 **95%**）就在本地拦截，返回 429，请求到不了 Anthropic。停在 98% 太薄，一发长对话容易打穿 100%
- 已经标定过「多少用量对应 1%」时，若**这一发**预估会越过预留线，也会在本地拦住
- **保活**继续用最后 5%（`max_tokens=1` + cache read）
...
| `reserve_percent` | 5 | CPA 对话在 5 小时窗口还剩这么多时停住（默认停在 95%）。2% 太薄，一发容易打穿 |

...
4. 状态页可单独勾选 / 取消 / 改名 / 忘记某一路。改过的名字在后续对话更新快照时会保留；留空保存则回到首条用户消息
...
- **CPA 对话**默认用到 **100%** 才在本地拦截（或这一发预估会打穿 100%）。`reserve_percent` 默认 **0**，不为保活另留百分比
- 已经标定过「多少用量对应 1%」时，若**这一发**预估会越过 100%（或你设的预留线），也会在本地拦住，避免超长上下文打穿窗口、随后被上游连着拒
- **保活**是 `max_tokens=1` + cache read，跟着对话窗口走；真的用到 100% / `rejected` 时保活也停，避免空打
- 若要提前停对话、把一截留给保活，把 `reserve_percent` 调大（例如 `5` 表示对话停在 95%）
...
| `reserve_percent` | 0 | CPA 对话提前多少个百分点停住。`0` 表示用到 100%（或这一发会打穿）才拦；想给保活留一截就把这个调大 |

...
- **CPA 对话**默认用到 **95%** 就在本地拦截，返回 429。停在 98% 太薄，一发长对话容易打穿 100%
- 状态页可以改「对话停在 x%」，范围 60–99，写到 `claude-cache-keepalive.settings.json`，重启 CPA 也还在
- 已经标定过「多少用量对应 1%」时，若**这一发**预估会越过停线，也会在本地拦住
- **保活**继续用停线后面那一截（`max_tokens=1` + cache read）
...
| `stop_percent` | 95 | CPA 对话在 5 小时用量到达这个百分比时停住。状态页可改；优先于 `reserve_percent` |
